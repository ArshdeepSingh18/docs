...
<Note>
Studio experiments follow the same [data retention](/langsmith/administration-overview#data-retention) rules as other experiments. By default, traces have base tier retention (14 days). However, traces will automatically upgrade to extended tier retention (400 days) if:

- The [dataset](/langsmith/evaluation-concepts#datasets) has [evaluators](/langsmith/evaluation-concepts#evaluators) configured, or
- [Feedback](/langsmith/observability-concepts#feedback) is added to any run in the experiment.
